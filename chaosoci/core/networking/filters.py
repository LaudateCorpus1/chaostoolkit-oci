...
__all__ = ["filter_nat_gateways", "filter_route_tables"]
...
def filter_nat_gateways(nats: List[NatGateway] = None,
                        filters: Dict[str, Any] = None) -> List[NatGateway]:
    """Return only those NAT gateways that match the filters provided."""
    nats = nats or None

    if nats is None:
        raise ActivityFailed('No NAT Gateways were found.')

    filters_set = {x for x in filters}

    available_filters_set = {x for x in nats[0].attribute_map}

    # Partial filtering may return nats we do not want. We avoid it.
    if not filters_set.issubset(available_filters_set):
        raise ActivityFailed('Some of the chosen filters were not found,'
                             ' we cannot continue.')

    # Walk the nats and find those that match the given filters.
    filtered = []
    for nat in nats:
        sentinel = True
        for attr, val in filters.items():
            if val != getattr(nat, attr, None):
                sentinel = False
                break
